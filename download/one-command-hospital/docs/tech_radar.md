...
|---|---|---|
| Purpose-built vector DBs (Qdrant, Weaviate, Milvus, pgvector) | **ADOPT (pgvector form)** | Hybrid BM25+vector is already the design; SBERT backend exists behind a flag. Run vectors in **Postgres (pgvector)** inside the existing `data` tier instead of adding a new stateful service: one less thing to secure/backup/patch, same ANN performance at hospital-corpus scale (<100k chunks). A dedicated Qdrant becomes rational only past ~1M chunks or multi-tenant scale. |
...
| Structured-output / constrained decoding (vLLM guided JSON, outlines) | **ADOPT** | Force the generator's answer schema (citations required, NOT_COVERED literal) at the decoding level instead of trusting the prompt. Removes an entire hallucination class mechanically. Small change, vLLM-native. |
...
| Tech | Verdict | Assessment |
|---|---|---|
| MedMemedium/Clinical-bold FHIR `$process-message` + CDS Hooks (`patient-view`, `order-sign`) | **ADOPT (CDS Hooks)** | CDS Hooks is how real EHRs (Epic, Cerner) surface decision support natively — bigger realism win than any model upgrade. `patient-view` hook = the copilot appears inside the chart. Sandbox already speaks FHIR R4; a CDS Hooks service is a thin facade on the mediator. |
...
1. **ADOPT now (in-repo, no GPU):** guided/constrained decoding config shipped
   alongside the vLLM profile; CDS Hooks `patient-view` facade on the
   mediator; pgvector volume wired into the compose `data` tier (dormant until
   the GPU pilot flips `VECTOR_BACKEND=sbert`).
